...
1. **`epic-merge.yml` (reusable) — nuevo handler `postStateClose` + rama de evento `issues`.** El script gana una rama disjunta del camino de merge (no hay PR): al cerrarse un issue, si es `epica`, respeta kill-switches (`pause-agents`/`human-needed`) y actúa SOLO sobre la clase evidenciada «terminal por estado» — `estado:cierre-pendiente-humano` (AP-020) o rastro `creator-alcance-completo-materializado*`. Consume el sentinel con la MISMA lógica del merge: `epic-audit` ⇒ `createAuditIdempotent` (idempotente por título/claim, ancla el claim en el issue cerrado) + `armQueue`; `launch-next: #N` ⇒ des-bloqueo mecánico (`stalled`/`human-needed`) + arm con `epic-auto-launch`; `epic-done` ⇒ `armQueue`. El arm resultante pasa por el guard serial normal de `claude-code.yml` (se encola con `en-cola` si la serie está ocupada) y respeta los mismos kill-switches. Marcador de rastro+dedupe NUEVO `<!-- epic-state-close-processed -->` (procesado una vez por issue, superviviente a reopen/reclose).
