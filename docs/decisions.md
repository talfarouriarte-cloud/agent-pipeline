...
**Decisión (del propietario, ejecutada por el Creator del central; AP-012).** Rama complementaria (else-case) del clasificador de terminales de PR-loop, en el MISMO post-step (`Auto-label` de `claude-code.yml`) que hoy ejecuta el `turn-close-failsafe`:
...
3. **Vocabulario.** Fila de `<!-- escalada-materializada-con-pr -->` añadida a `protocol.md` (gemelo con-PR de `escalada-materializada`), en el MISMO cambio.
