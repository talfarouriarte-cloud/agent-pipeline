...
## AP-036 — La escalada NO-declarada del Creator se materializa como el MISMO terminal que su gemela declarada (`stalled` ⇒ architect-resolve POR EVENTO), no `human-needed`: la costura es el ESTADO, no la prosa (cierra la clase que tres fixes de canal no sellaron)

**Contexto.** La rama «escalada no-declarada» del post-step de terminales del Creator (`claude-code.yml`, bloque v4/central#34) materializaba `human-needed` — terminal de cortacircuito humano — para EXACTAMENTE el mismo estado que su gemela declarada (`[CREATOR-BLOCKED]`/`<!-- creator-blocked -->`) materializa como `stalled` ⇒ architect-resolve POR EVENTO. La única diferencia entre ambas ramas era la omisión de prosa del Creator, y esa omisión midió **6 ocurrencias a través de TRES fixes de canal sucesivos** (AP-011 → belt AP-028 → texto anclado AP-035): 3 POST-AP-028 (finplan#1484/#1510/#1531) y la 1ª POST-AP-035 (finplan#1532 — la sesión de 2026-07-19T19:59Z corrió con AP-035 desplegado desde las 17:17Z, commit `6cbae7ac`, y aun así cerró sin token). En la épica ADR-229·R·2 la clase disparó DOS veces y cada disparo costó intervención humana: finplan#1531 (escalada legítima y bien anclada → `human-needed` → cirugía manual `human-needed`→`stalled` → architect-resolve la resolvió EN AUTÓNOMO 6 min después, `<!-- autonomous-decision -->`, Opción A pinada, PR #1549 LGTM directo — **la maquinaria autónoma demostró resolver exactamente este estado en cuanto alguien se lo entregó; lo único que faltó fue la transición**) y finplan#1532 (escalada legítima con 2 decisiones no-derivables → `human-needed` → **12h de stall nocturno** → resolución humana + re-arm manual a las 08:08Z).

**5-whys.** El re-arm humano ocurre porque `human-needed` solo lo limpia un humano ← el fallback elige `human-needed` ← la rama ya DERIVÓ el estado correctamente (serie ocupada + `success` + sin commits + sin PR = «escalada no-declarada») pero materializa un terminal distinto al de su gemela declarada ← **causa raíz: la transición materializada para el estado derivado es la equivocada** — no la omisión del Creator (tres fixes de canal no la han sellado y no la sellarán al 100%; doctrina push-primario wmcb#38: el estado, no la prosa, es la costura). AP-035 previó este escalón en su cláusula «Falsable»; esta es su enmienda.

**Decisión (del propietario, ejecutada por el Creator del central).** En la rama no-declarada del post-step (tras el check de `epic-partial-relaunch-por-estado`), sustituir `human-needed` por la MISMA materialización de la rama declarada: `stalled` con PAT (⇒ architect-resolve POR EVENTO) + retirada de `serial-activo` + `popQueue`, reescribiendo el texto del comentario `escalada-materializada` para reflejarlo (conservando el marcador como rastro para el Auditor). La omisión de prosa deja de costar horas: queda como métrica informativa de legibilidad, igual que `@reviewer` bajo push-primario. Segundo cambio en el MISMO PR (append-only): añadir la fila `<!-- escalada-materializada -->` a `protocol.md` — el marcador se emite desde v4 (central#34) y NO estaba en la tabla de vocabulario (violación del append-only; su ausencia contribuyó a que el Auditor de finplan#1556 no lo barriera).

**Coste y riesgo.** Coste: edición acotada de una rama del post-step + una fila de `protocol.md`. Riesgo 1 (sesión genuinamente conversacional clasificada como escalada): ya ocurre hoy — la rama exige serie ocupada + sin commits + sin PR; con el cambio la paga un run de architect-resolve (diagnostica y no-opa/escala) en vez de un `human-needed` espurio; mejora en ambos casos. Riesgo 2 (escalada que SÍ requiere humano): architect-resolve la rebota (doble rebote ⇒ `human-needed`), coste un run; hoy el caso común (resoluble en autónomo, como #1531 demostró) paga 1-12h de humano. Riesgo 3 (interacción de labels, clase conocida #3 del pipeline-map): `stalled` por PAT es la MISMA arista que la rama declarada ya usa (`labeled` ⇒ architect-resolve por evento); ningún writer nuevo, cero impacto en `reviewer.yml`/`epic-merge`. Breaker intacto: el progress-aware final breaker sigue cubriendo el thrash (re-escalada sin merges intermedios ⇒ `human-needed`).

**Falsable.** El re-arm humano de escaladas no-declaradas debe caer a 0: toda escalada no-declarada legítima aterriza en architect-resolve por evento (autónomo) sin tocar a un humano salvo doble rebote. Si architect-resolve rebota a `human-needed` con frecuencia > la de las escaladas declaradas, la clasificación mecánica (serie + success + sin commits + sin PR) está sobre-capturando conversacionales y hay que estrecharla.

**Superficie tocada.** `.github/workflows/claude-code.yml` (rama no-declarada del post-step: `human-needed`→`stalled` con PAT + reescritura del comentario + comentario de canal AP-035 actualizado a 6 ocurrencias), `vendored/docs-agents/protocol.md` (fila `escalada-materializada`, append-only con nota datada).
