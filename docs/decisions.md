...
**Contrato `workflow_call`.** Sin cambios de superficie: el helper es interno al `script:` del step, reusa el PAT `REVIEWER_GITHUB_TOKEN` ya presente; no añade inputs/secrets/labels (los marcadores son comentarios HTML, no labels) ⇒ `templates/workflow-contracts.json` y `templates/labels-usage.json` intactos (AP-003, AP-022). `vendored/creator.md` se sirve por graft en runtime (AP-009) — llega a finplan/wmcb en su siguiente run sin gradualidad; el cambio es aditivo (una vía de fallback nueva, no altera caminos existentes).
