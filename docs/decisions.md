...
3. **Vocabulario + Auditor.** `protocol.md` (append-only): filas de la línea `pre-épica:` y del marcador `dry-run-ausente-stall`. El Auditor contrasta veredicto observado vs declarado: una declaración falsa pasa a ser evidencia visible en vez de nada. `watchdog.md`: mandato de architect-resolve para el tipo `dry-run-ausente-stall` (ejecuta el dry-run, añade las líneas + cita de cláusula, re-arma; corrige el invariante si contradice su cláusula; o rula `invariantes-na`).
