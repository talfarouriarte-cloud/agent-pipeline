...
> **Enmienda (2026-07-15) — incidente `startup_failure` de flota: el desdoble de tokens de §338 y el «permiso de lectura» de §346 quedan superados.** La implementación original (#57) montó la lectura de runs sobre el `GITHUB_TOKEN` default y para ello añadió `actions: read` a los `permissions` del job del reusable `claude-code.yml`. Un callee de `workflow_call` NO puede pedir más permisos que su caller: todo stub que declara un bloque `permissions` explícito **sin** `actions` resuelve a `actions: none`, de modo que el job pidiendo `actions: read` EXCEDE al caller y GitHub rechaza el arranque con `startup_failure` (sin jobs ni logs). Como los stubs referencian `@main`, el fallo fue de flota y simultáneo (central 22:57:54Z, finplan 23:00:38Z, wmcb en su siguiente trigger). **Fix (#60, callee-only, cero cambios en stubs):** se retira `actions: read` del job y la lectura de runs pasa del `GITHUB_TOKEN` default al PAT `REVIEWER_GITHUB_TOKEN` (raw fetch a `/actions/runs?head_sha=…`, el mismo canal que ya escribía el label). En consecuencia, los tres asertos del paréntesis de §338 dejan de valer: (a) la lectura ya NO usa el token default; (b) `actions: read` ya NO está en el job; (c) el desdoble **ya no es «el mismo del modo e) del watchdog»** — watchdog sigue leyendo con el token default + `actions: read` porque su stub concede `actions: write` (dentro de límites), asimetría que explica por qué el incidente solo tumbó los runs de «Claude Code» y dejó al watchdog en pie. Endurecimiento correlativo: `found()` pasa a trivaluado — si la lectura falla (p. ej. el PAT careciera de `actions: read`), el step NO decide (warning + salida) en vez de materializar sobre lectura fallida (que doblaría reviews); la red sigue siendo el dispatcher del watchdog (ADR-217), comportamiento pre-#57. La **decisión** de este AP (materializar Creator→Reviewer por estado en la apertura, con dedupe) queda intacta y correcta; lo que se rectifica es el **mecanismo** de §338 y la enumeración de §346 (ese «permiso de lectura» ya no existe: el belt no añade permiso alguno al job). **No reintroducir `actions: read` en los `permissions` del job del reusable** — es exactamente el landmine de este incidente. Señal de clase diferida: los `permissions` del job del reusable son superficie de contrato con los stubs y `templates/workflow-contracts.json` no la modela (los inputs/secrets de `workflow_call` no cambian ⇒ AP-003 no cubre este eje).
...
**Reversibilidad.** Alta: un post-step, un input con default y —en la implementación original— un permiso de lectura en el job; se revierte quitando el step. *(Ver Enmienda 2026-07-15: el fix callee-only movió la lectura al PAT y retiró ese permiso; el belt ya no añade ningún `actions:` al job.)*
