...
3. **Arms posteriores pasan en silencio.** Con el bloque presente (`hasInvariants`), con `invariantes-na` declarado por comentario de confianza, o siendo un eslabón encadenado (`epic-auto-launch`, cuya casa canónica del bloque queda aguas arriba e invisible al traversal forward): cero re-avisos y cero re-stalls (converge con el dedup de AP-018 §2).
...
**Scope del stall al PRIMER eslabón.** Los eslabones aguas abajo se arman por `epic-auto-launch` (epic-merge) y quedan exentos: la decisión de invariantes ya se tomó en el primer eslabón, y su bloque (casa canónica = issue de épica, aguas arriba) es invisible al traversal forward desde un eslabón intermedio — sin la exención se re-stallearía la cadena a mitad de vuelo. Un primer eslabón funcional se arma por el Auditor/humano (ARM_TOKEN, `@claude`), no por `epic-auto-launch`, así que la exención no lo cubre por error.
