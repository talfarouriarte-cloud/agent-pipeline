...
## AP-046 — La salida de escalada del PR-loop se materializa por ESTADO (gemelo con-PR de `escalada-materializada`/AP-036): rama else del `turn-close-failsafe` que cierra el «turno-de-nadie» sin depender de que el Creator emita un token
...
**Decisión (del propietario, ADOPTADA; implementación PENDIENTE — ver «Estado de implementación» abajo; AP-012).** Rama complementaria (else-case) del clasificador de terminales de PR-loop, en el MISMO post-step (`Auto-label` de `claude-code.yml`) que hoy ejecuta el `turn-close-failsafe`:
...
3. **Vocabulario.** Fila de `<!-- escalada-materializada-con-pr -->` añadida a `protocol.md` (gemelo con-PR de `escalada-materializada`), **marcada como productor PLANIFICADO** (no vivo) hasta que la rama aterrice — ver «Estado de implementación».

**Estado de implementación (2026-07-22 — DOCTRINA ADOPTADA, MECANISMO NO ATERRIZADO).** La rama else descrita en la Decisión §1 **NO existe todavía** en `claude-code.yml`. Verificado sobre el HEAD de este PR: `grep escalada-materializada-con-pr` sobre `claude-code.yml` = 0 hits; para el caso exacto que este AP ataca —`success` + trigger `ping-creator` + sin push posterior al veredicto + sin tag de cierre anclado + sin token de escalada por ninguno de los dos canales— el `turn-close-failsafe` sigue cayendo al `core.notice` de «sin tag» (`claude-code.yml:1729-1733`, no-op) y el «turno-de-nadie» lo sigue rescatando el cron del Watchdog. Esta entrada registra la DECISIÓN por delante del código (excepción consciente a la norma «código-primero, doc-después» del corpus) porque la sesión que la produjo —el Creator de #128— terminó parcial (murió antes de declarar alcance; el PR lo abrió el post-step por estado) y una sesión de agente NO puede modificar `.github/workflows/`. **Tracking del mecanismo restante:** el issue **#128 DEBE permanecer abierto** con el alcance «landar la rama else de `turn-close-failsafe` en `claude-code.yml`» como issue de seguimiento del Architect; hasta que aterrice, ni el corpus ni ningún agente (architect-resolve, Auditor, 5-whys) deben leer AP-046 como un fix vigente de la clase «token de escalada no convertido» — la costura sigue cubierta SOLO por el cron del Watchdog. Este bloque se retira (y la cabecera de la Decisión pasa a «ejecutada») en el MISMO cambio que introduzca la rama else.
...
**Falsable.** PRs con veredicto REVIEW vigente que queden en «turno-de-nadie» rescatados por cron del Watchdog (`watchdog-rearm`) por esta clase: deben caer a 0 (la transición pasa a ser por evento en el mismo job). Si architect-resolve rula mal ≥2 veces sobre la prosa de un cierre así, el siguiente escalón es estructurar el cierre del Creator por template inyectado en el arm (mismo criterio que AP-036), no volver a un canal declarativo.

**Reversibilidad.** Alta: la rama else es un bloque autocontenido dentro del `turn-close-failsafe` que se elimina volviendo al `core.notice` de «sin tag»; la fila de `protocol.md` se retira con esta entrada. Sin migración de datos ni de estado.
