...
**Decisión (del propietario, EJECUTADA — rama else aterrizada en `claude-code.yml`, commit `d9978c5` / PR #135; ver «Estado de implementación» abajo; AP-012).** Rama complementaria (else-case) del clasificador de terminales de PR-loop, en el MISMO post-step (`Auto-label` de `claude-code.yml`) que hoy ejecuta el `turn-close-failsafe`:
...
3. **Vocabulario.** Fila de `<!-- escalada-materializada-con-pr -->` añadida a `protocol.md` (gemelo con-PR de `escalada-materializada`), **productor VIVO** (rama else aterrizada, commit `d9978c5`) — ver «Estado de implementación».

**Estado de implementación (2026-07-22 — EJECUTADA).** La rama else descrita en la Decisión §1 **aterrizó** en `claude-code.yml` (step `Auto-label` del `turn-close-failsafe`) con el commit `d9978c5` (PR #135, mergeado a `main` en `2932eae`). Verificado: `grep escalada-materializada-con-pr` sobre `claude-code.yml` = 5 hits (detección/dedupe HTML-anclada, materialización de labels, cuerpo del marcador con huella de head SHA). Para el caso exacto que este AP ataca —`success` + trigger `ping-creator` + sin push posterior al veredicto + sin tag de cierre anclado + sin token de escalada por ninguno de los dos canales— el `turn-close-failsafe` ya **no** cae al `core.notice` no-op: materializa en el instante `estado:esperando-architect` (el dispatcher de turno CEDE, AP-017) + `stalled` (con el PAT `REVIEWER_GITHUB_TOKEN` ⇒ architect-resolve POR EVENTO, sin cron) + marcador de diagnóstico `<!-- escalada-materializada-con-pr -->`. **AP-046 es a partir de aquí un fix VIGENTE** de la clase «token de escalada no convertido»: architect-resolve, Auditor y 5-whys deben leerlo como tal (la costura ya NO depende solo del cron del Watchdog). La fila del marcador en `protocol.md` §3 pasa a productor VIVO (`docs/agents/protocol.md:41` y su copia vendored en sync, AP-009). El seguimiento del mecanismo en #128 queda cerrado por este aterrizaje (era el único alcance restante del issue).
