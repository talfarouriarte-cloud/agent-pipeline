...
**Decisión (directriz del propietario, 2026-07-18).** El check-and-take es el PRIMER paso de todo run despachado y su unicidad es responsabilidad del RUN en su arranque, no del actor que arma ni de una vigilancia externa (frágil por diseño). El guard `check_serial` ya ES ese primer paso (step pre-despacho en el mismo job que la sesión). Se ENTREGA **Change A**; el cinturón TOCTOU (**Change B**) queda DEFERIDO por un fallo de diseño detectado en revisión adversarial (ver abajo).
...
**Diseño corregido PROPUESTO (para decisión del propietario/Architect, NO implementado aquí).** Cerrar la ventana de dos-arranques-simultáneos con un ganador ÚNICO determinista, sin depender del orden de llegada: en la rama «serie libre», leer los runs Creator `in_progress` del MISMO issue (filtrando por `r.name === context.workflow` — el nombre del workflow caller, robusto en contexto reusable — Y `display_title == issue.title`); el ganador es el de `run_id` MENOR; **solo el ganador escribe el flag y procede, los demás ceden SIN escribir**. Así todos los contendientes calculan el mismo ganador y el flag lo escribe únicamente quien procede (no hay orfanato; a lo sumo, si el ganador cae en la ventana, se pierde el arm — lo recupera el watchdog/cola, sin flag colgado). Alternativa a evaluar: `concurrency` nativo de Actions en el stub caller — pero `cancel-in-progress: true` cancela al ANTIGUO (dirección opuesta) y `false` sólo encola; ninguna encaja tal cual, y vive fuera del reusable. Requiere sign-off por tratarse de una primitiva de concurrencia (criterio `[NEEDS-HUMAN]` del CLAUDE.md) sobre un workflow que despliega a dos consumidores.
...
**Reversibilidad.** Alta: Change A se revierte restaurando `flagged.filter(i => i.number !== issue.number)` y esta entrada.
