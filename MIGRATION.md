...
**Coverage of the runtime notice — read the limits, they are not symmetric.** It fires only when the received value is *below* the default. Three cases it does NOT see: a pin **equal** to the default (legal today, silently stale the day the AP is amended), a pin **above** it (deliberate over-provisioning), and any pin of an input not declared `lesson_bearing`. It also cannot see whether a pin is annotated — annotations are comments in the consumer's stub and never leave it. The notice is a belt for the one case that already cost an incident, not a general pin auditor.

**Default rule: do not pin them.** Leave the line out and the central default rules, so future lessons arrive on their own — this is the only thing that protects the "equal to the default" case, which no mechanism catches. If a repo genuinely needs a different value, the pin ships **with an annotation** stating *why this repo overrides a lesson-bearing input* (not merely why the value is what it is) plus the AP reference. A pin without annotation is a defect by doctrine; nothing in CI will tell you.
