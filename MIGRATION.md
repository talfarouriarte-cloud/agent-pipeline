...
> **Pinning rule (AP-052, clamp AP-061).** Every `with:` line is an override of a central default, so every line has to earn its place: fill in what is *instance truth* (branch names, CI job name, runner, caps you deliberately chose) and leave everything else out. Never pin a **lesson-bearing** input (§4) — its default carries an AP behind it. Since AP-061 a lesson-bearing pin **below** the central default no longer governs: the reusable takes `max(pin, default)` and records the clamp, so the line is inert *and* misleading about your own configuration. If you genuinely need a lower budget, pass `budget_pin_forzado: true` next to it and own the consequence. Either way the line ships with an annotation (reason + AP reference + date); a pin without annotation is a defect **by doctrine — nothing enforces it**, because the reusable only receives input *values*, never the comments around them. A pin equal to or above the default is still invisible, and so is any pin of a non-lesson-bearing input. Read the mechanism's coverage in §4 before assuming a wrong pin will announce itself.
...
Pinning one **below** the central default used to silently cancel a fix that was already deployed — "a merged central PR IS the deployment" (README §13) quietly stopped being true for that repo. AP-052 made that divergence visible (`::notice` + job-summary table + the offending pin inline in the escalation comment). **AP-061 makes it inert:** the reusable now takes `max(pin, default)` on every lesson-bearing input, so a low pin no longer governs. The annotation stays, and gains the outcome:
...
**The valve — when you genuinely want less.** A lower budget is a legitimate choice (cost control). To make one actually govern, pass `budget_pin_forzado: true` alongside the pin; the annotation then reads `[pin forzado]` and the escalation comment names the pin as the prime suspect for a budget death. The valve is per *reusable*, not per input: forcing one budget forces them all in that workflow.

**Coverage — read the limits, they are still not symmetric.** The clamp compares against the default the central serves **today** (the literal ships inside the reusable, resolved at `@main` on every run), never the one current when your stub was written. What it does and does not reach:

- A pin **below** the default: **prevented** (clamped up) and recorded. This is the case that cost finplan#1674 and finplan#1704.
- A pin **equal** to the default, the day its AP is amended **upwards**: also **prevented** — the pin is now below the new default, so it clamps on the very first run. This was AP-052's expensive blind spot ("the session already ran on the pre-lesson budget") and AP-061 closes it.
- A pin **above** the default: untouched by design — over-provisioning is a deliberate choice and the clamp only ever raises. Permanent blind spot unchanged: if an AP is amended **downwards**, yesterday's pin lands above the new value and nothing flags it. The cost is money, not a lost lesson.
- Any pin of an input not declared `lesson_bearing`: invisible, unclamped. Nothing here audits `creator_max_turns` or `lookback_min` — they carry no declared lesson.
- Whether a pin is **annotated**: still unenforced. Annotations are comments in your stub and never leave it; the reusable receives values only.
