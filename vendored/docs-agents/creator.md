...
**Cómo elegir el literal (evita el falso positivo).** No grepees el número desnudo (`50`, `400`): grepea el número **con su unidad o su contexto** — `50 µs`, `N=5.000`, `p90`, o el nombre de la constante/helper que lo encapsula. **La unidad sola no basta: ancla también el borde IZQUIERDO** (`grep -E "(^|[^0-9])50 µs"`), porque el literal con unidad sigue siendo subcadena de otro número con la misma unidad — medido al redactar esta regla: `50 µs` casa dentro de `650 µs/frame`, un número de OTRA cláusula del mismo ADR. El objetivo es que el grep sea legible como evidencia, no que salga vacío a base de estrechar el patrón: un patrón tan específico que no pueda casar nada es evidencia falsa, y así hay que declararlo si dudas.
