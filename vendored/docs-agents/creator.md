...
|---|---|---|---|
| Paras **SIN PR abierto**, decisión derivable-por-Architect (serie/emisión ausente del estado derivado, hueco de dimensionado, ambigüedad de ADR que un `autonomous-decision` cierra) | `<!-- creator-blocked -->` (HTML) | `stalled` (PAT ⇒ architect-resolve por evento) + retira `serial-activo` + pop de cola | Comentario SEPARADO vía `gh api` en el **ISSUE** (ver § Canal) |
| Paras **CON PR abierto**, decisión derivable-por-Architect desde dentro del loop (gate numérico de ADR tipo A3′, ambigüedad de diseño) | `<!-- creator-escalated -->` (HTML) | `stalled` (PAT ⇒ architect-resolve por evento) + `estado:esperando-architect` (el dispatcher de turno cede) | Comentario SEPARADO vía `gh api` en el **PR** (ver § Canal) |
...
Reglas duras: `creator-escalated` es el complemento EXACTO de `creator-blocked` — **JAMÁS emitas `creator-escalated` sin PR abierto** (usa `creator-blocked`) ni **`creator-blocked` con PR abierto** (usa `creator-escalated`); **JAMÁS uses ninguno de los dos para una decisión humana** (eso es `[NEEDS-HUMAN]`, intacto). Ambos marcadores son **HTML anclado** — el post-step los detecta como comentario, nunca como substring de prosa (clase PR #1133): escríbelos en su propia línea y por el **canal separado vía `gh api`** (ver § Canal más abajo — NO en el tracking comment, que pierde el HTML). La decisión concreta que escalas va en la PROSA del **tracking comment de cierre** (qué gate, qué ADR, qué ambigüedad) para que architect-resolve rule sin re-derivarla: la prosa (texto) sobrevive el tracking comment; solo el HTML se pierde, y por eso el marcador viaja aparte.
...
### Canal de los marcadores terminales: comentario SEPARADO vía `gh api` (AP-028)

Los tres marcadores terminales (`creator-escalated`, `creator-blocked`, `creator-alcance-completo`) son **HTML**, y **el tracking comment de `claude[bot]`** — el que la action REESCRIBE al terminar («Claude finished…») — **elimina todos los comentarios HTML** (AP-013: 3/3 omisiones medidas; repesca finplan#1467: una escalada correcta y correctamente declarada en prosa se desmaterializó porque su marcador viajaba en ese canal, y quedó `turno-de-nadie` 1h38m hasta el rescate del Watchdog). **Por eso NO escribas el marcador dentro de tu comentario de cierre** (ese es el tracking comment): publícalo como **comentario SEPARADO vía `gh api`** (canal que demostradamente conserva HTML — mismo remedio que AP-013 aplicó al `audit-verdict`):

```bash
gh api repos/{owner}/{repo}/issues/<N>/comments -f body='<!-- creator-escalated -->'
```

(en el **PR** para `creator-escalated`; en el **ISSUE** para `creator-blocked` y `creator-alcance-completo` — la columna «Dónde lo emites» de la tabla lo fija). El tracking comment de cierre queda como **prosa legible** que explica la decisión (qué gate, qué ADR, qué ambigüedad) para que architect-resolve rule sin re-derivarla — pero el ESTADO lo porta el comentario separado, no la prosa.
