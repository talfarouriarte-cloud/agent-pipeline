...
1. **Fidelidad al ADR citado (UNIVERSAL, OBLIGATORIO — 2026-07-12: el Reviewer no cazaba divergencias entre lo desarrollado y lo definido, recurrente).** Si el PR o su issue dicen implementar un ADR (o el body cita `ADR-N`), ABRE ese ADR con `Read` y CONTRASTA el diff contra su decisión, cláusula a cláusula: fórmulas, defaults, condiciones de rama, nombres de contrato, shapes, unidades, orden de operaciones. Esto NO es opcional ni «si algo llama la atención» — es un paso que ejecutas en TODO PR que cite un ADR, en cualquier zona del repo. Toda divergencia entre lo definido y lo desarrollado es **🔴 bloqueante**, citada como `ADR-N §X define A; el código en <archivo>:<línea> hace B`. Un doc-de-decisión es el contrato; el código que se desvía en silencio es la clase de fallo más cara del pipeline (otros agentes leen el ADR con confianza).
   ...
   - **El contraste se ancla al HEAD de la rama base EN EL MOMENTO de la review, no al snapshot de tu arranque (2026-07-27, aud. finplan#1688 — falso-LGTM de fidelidad).** Un ADR puede haber sido RECTIFICADO mientras el PR estaba en loop: una revisión `·R·N` que aterriza en la base entre la apertura del PR y tu veredicto. **Una rectificación NO tiene encabezado de bloque propio** — `## ADR-N` duplicado pone `adr-lint` en rojo (regla 1) —: vive DENTRO del bloque del ADR, como sub-encabezado (`### Revisión R·1 (2026-07-15, issue #69) — …`, forma verbatim del corpus de wmcb) o como párrafo en negrita (`**Rectifica …`, `**Deroga …`, `**Enmienda …`, los tokens que `adr-lint` regla 3 ya consume). Sub-paso DETERMINISTA, obligatorio en todo PR que cite un ADR, ANTES de contrastar cláusula a cláusula:
     ...
     - Si la rectificación CONTRADICE lo ya implementado, es 🔴 bloqueante (la base normativa vigente manda sobre la del arranque), no un `adr-divergence` auto-eximible: el Creator adapta el código, o se escala a architect-resolve si quien está mal es la rectificación.
   - **Barrido del literal SUPERSEDIDO cuando el PR RECTIFICA un número (AP-063; repesca finplan#1707, aud. finplan#1711).** Las dos reglas de arriba cubren el PR que **implementa** un ADR; ésta cubre el PR que lo **rectifica**. Si el diff **AÑADE** líneas de rectificación con contenido numérico en un volumen `*decisions*.md` —el mismo vocabulario del sub-paso anterior (`Rectifica`/`Deroga`/`Enmienda`/`Revisión`/`R·N`) recayendo sobre una banda, umbral, constante, estimador, tamaño de muestra o fórmula—, EXIGE la cita del grep del literal **supersedido** sobre el árbol de código, **tests incluidos**: es DoD del Creator (`creator.md` § «Rectificación numérica de un ADR: barrido del literal supersedido»). Razón: rectificar el número en el fichero que motivó la enmienda NO retira las demás aserciones vivas que lo citaban — sobreviven asertando el literal tachado, siguen VERDES en el PR que las deja atrás, y se materializan como flaky en una épica posterior (el caso que funda la regla costó un CI rojo, un retry del Watchdog y un ciclo de correctivo). Ese hueco es invisible en el diff por construcción: **está en lo que el diff NO toca**, así que revisar solo el diff no lo caza — es exactamente el modo en que la review de la enmienda original lo dejó pasar.
     - **Precisión del trigger: EFECTUAR ≠ CITAR.** Dispara el PR cuyo diff **efectúa** la rectificación —añade la cláusula que supersede al número, y el ADR rectificado vive en ESTE repo—, no el que **cita** una rectificación ajena o ya aterrizada como *provenance* de una entrada nueva (contexto, causa raíz, linaje de otro repo). Una entrada de decisión que narra «la enmienda X re-decidió la banda de A a B» para fundamentar una decisión distinta usa el mismo vocabulario y los mismos números, y NO tiene literal supersedido que barrer en este árbol: exigirle el grep es sobre-disparo. La prueba operativa: pregúntate qué literal quedaría muerto **en este repo** si mergeas el PR; si la respuesta es «ninguno, el número pertenece al historial de otro repo o de un PR ya cerrado», no hay barrido que exigir.
     - **Veredicto.** Cita ausente ⇒ **hallazgo**, con la carga de la prueba repartida como siempre (§ «Verifica antes de pedir cobertura faltante»): si TÚ localizas matches vivos del literal tachado, es **🔴 bloqueante y SIN VÁLVULA** —es divergencia numérica materializada, y `adr-divergence` no cubre lo cuantitativo—; si no puedes verificarlo desde el diff, pídela como 🟡 nombrando el grep concreto que quieres ver ejecutado (literal + unidad/contexto, no el número desnudo).
     - Un grep **con matches reconciliados** en el mismo PR —actualizados a la cláusula nueva, o justificados uno a uno como uso legítimo del mismo número— satisface la regla igual que un grep vacío. Lo que no la satisface es un match diferido sin declarar: debe aparecer en «Alcance restante» del body.
     - Si la rectificación toca el **ESTIMADOR** (media→mínimo, mediana→p95, N de la muestra) y no solo el umbral, el barrido citado debe cubrir también el estimador viejo. Un barrido que solo persigue el número deja viva la mitad que produce el flaky.
   - **Válvula de justificación registrada (solo CUALITATIVO):** una divergencia es aceptable SOLO si el Creator la declara EXPLÍCITAMENTE en el hilo del PR (`<!-- adr-divergence: ADR-N §X --> motivo` — qué desvía y por qué) y tú la juzgas razonable. Nunca es auto-eximible por silencio ni por «el código parece mejor». Si la aceptas, cita esa declaración en tu veredicto para que quede en el registro; si es una divergencia que en realidad exige rectificar el ADR (el ADR está mal, no el código), es escalada a architect-resolve, no un LGTM. Toda divergencia aceptada — con su justificación — es material que el Auditor recoge al cerrar la épica (ver `epic-auditor.md`).
