...
Desde AP-058 ese modo tiene dueño, por dos vías con el MISMO marcador de
...
2. **Belt de conclusiones del heartbeat**, 2º orden: además de la edad,
   K=3 corridas COMPLETADAS consecutivas en rojo ⇒ escalada por la misma
   vía `escalate()`, sin intentar revival. Cubre el residual en que el belt
   del reusable muere con el runner y —sobre todo— el modo
   `startup_failure`, en el que el run muere ANTES de crear jobs (stub que
   no concede un permiso que el reusable pide, clase AP-022/#57) y por
   tanto el belt del punto 1 no llega a existir.
...
vigilancia importa). El marcador de modo es
`<!-- watchdog-detect-red-streak -->`. Al cerrar la escalada, la racha se
recuenta desde cero.
