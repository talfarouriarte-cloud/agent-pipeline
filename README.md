...
**Lesson-bearing defaults in release notes (AP-052).** A default that changes because of a lesson learned is not an ordinary functional delta: consumers that *pin* that input keep running the pre-lesson value, and "the upload IS the release" quietly stops being true for them. So when a milestone contains such a change, its release notes name the input, the AP that records the lesson, the old and new value, and the consumers known to pin it (sweep their stubs — `templates/workflow-contracts.json` `lesson_bearing` is the list of inputs to sweep for). The runtime belt is in the reusable itself, which flags every run that receives such an input *below* the current default — a partial belt, and late: a pin equal to the default reads as compliant today, and once the AP is amended upwards the notice does fire — but only from a run that has *already* executed on the pre-lesson value (and never at all if the amendment goes downwards). That is exactly the case the sweep exists to catch **before** it costs a run. The release note is what makes that sweep happen once instead of one incident at a time. Precedent: `reviewer_max_turns` 50→80 (AP-025) reached the central and never reached finplan, whose stub had pinned 50 — three no-verdict Reviewer deaths and two human interventions later, a human wrote the annotation by hand.
