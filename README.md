...
| **Creator** | Claude Code GitHub Action, fired by `@claude` | Arm-time deterministic guards before any agent session burns: chain integrity (reachable chain must end in an epic-audit sentinel) and panel-consumed (a MANUAL arm of an epic issue is refused — `stalled` + `panel-sin-consumir` — while `auditoria`/`process-proposal` issues sit open in THAT repo; panel data is always local to the arming repo, so open audits in one consumer never block launches in another; exemptions: epic-merge auto-launch arms, correctives citing a live audit, a per-panel declared-consumed marker `<!-- panel-consumido: #N -->` in any trusted comment (AP-019 — the guard reads declared state instead of requiring a `close` no agent's allowlist can perform), explicit `<!-- panel-ok -->` full override). Implements exactly one issue per PR. If scope doesn't fit one green PR: delivers a hybrid PR marked `partial-pr` with a verifiable remaining-scope report — never restructures the chain | Chain surgery (creating/re-chaining issues); running the full test suite (CI's job); opening a PR without a polarity marker |
...
**Dual mode (owner decisions, 2026-07-12): owner-operated consumers pin `@main`; tags are documented milestones.** With one operator, the human gate already lives at the central commit — the upload IS the release; a downstream re-pin step decided nothing and left mixed-version states latent (a consumer ran 10 days on a stale watchdog with a known bug because re-pinning was per-file-per-repo). Rollback = re-upload the previous file. Tags resume as documented milestones: cut only after the pending changes survive real traffic in the owner's repos (post-audit of the first epic exercising them), each with release notes stating the functional delta and its motivation. Nothing consumes tags today; they are the ready-made, pre-documented history for any future external consumer. Published tags are immutable — content change = new tag.

**Lesson-bearing defaults in release notes (AP-052).** A default that changes because of a lesson learned is not an ordinary functional delta: consumers that *pin* that input keep running the pre-lesson value, and "the upload IS the release" quietly stops being true for them. So when a milestone contains such a change, its release notes name the input, the AP that records the lesson, the old and new value, and the consumers known to pin it (sweep their stubs — `templates/workflow-contracts.json` `lesson_bearing` is the list of inputs to sweep for). The runtime belt is in the reusable itself: since AP-061 it does not merely *flag* an input received below the current default, it **clamps** it to `max(pin, default)` and records the outcome — so the pre-lesson value never governs a run, whether the pin was written that way or drifted below by an upward amendment. Two limits survive, and both are the sweep's job: the clamp only ever raises (a downward amendment leaves yesterday's pin above the new default, unflagged forever), and it reaches only inputs actually declared `lesson_bearing`. A consumer that deliberately wants less passes `budget_pin_forzado: true` — itself worth naming in the sweep. The release note is what makes that sweep happen once instead of one incident at a time. Precedent, twice over: `reviewer_max_turns` 50→80 (AP-025) reached the central and never reached finplan, whose stub had pinned 50 — three no-verdict Reviewer deaths and two human interventions later, a human wrote the annotation by hand; four days after that annotation shipped, the same stub's `timeout_minutes: 15` killed a Reviewer session on finplan#1704 while emitting the AP-052 notice verbatim, two lines below that very comment. Visibility alone does not survive contact with the next parameter.
