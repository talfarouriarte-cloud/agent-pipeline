...
| **Watchdog** | Cron + workflow-completion events | Detects stalls and anomalies; deterministic retries (red CI, startup failures, broken handoffs); **architect-resolve stage**: on escalations, diagnoses and decides autonomously, re-dimensions oversized issues, de-stalls and re-arms | Creating scope; touching human-flagged items; workflows; branch promotion |
| **process-reviewer** | Fired when an audit completes | Reads the audit's process metrics across epics; publishes ≤2 evidence-backed process proposals; runs the **skill-optimization loop**: bounded edits to trainable skills' learning sections, reverted if the targeted failure type doesn't improve within 2 epics | Editing outside learning sections; arming anything; proposing without metric evidence |
...
6. **Stubs** pinned to a release tag.
