...
// El doble de red NO tiene guard con warning, y la asimetría con los dos de arriba
// es deliberada (🟡 2 de la review de AP-076). Aquellos protegen contra un árbol que
// legítimamente puede no traer el gate ni el belt; éste se commitea en el MISMO
// árbol que este banco y en el mismo commit, luego su ausencia solo puede ser una
// borradura. Y la línea del verde de abajo enumera la capa de RED como cobertura
// dada: un `::warning` arriba no desmiente un verde abajo —el argumento exacto con
// el que AP-075 obligó a declarar el modo FIXTURE en la línea del verde del gate—,
// luego un banco que no puede ejecutar su instrumento no puede reportar verde de lo
// que no ha medido. Es ROJO.
const DOBLE = resolve(REPO, 'scripts/lib/fetch-doble.mjs');
if (!existsSync(DOBLE)) {
  console.error(
    'CHECK-RESOLVE-CORPUS-BANK ROJO: falta `scripts/lib/fetch-doble.mjs` — los casos de la capa de RED\n' +
    '  (`barrer()`: paginación, `desc`, timeout, fail-open por HTTP y la deriva de paginación) NO se pueden\n' +
    '  ejecutar, y este banco no puede dar por medida una capa que no ha ejecutado.\n' +
    '  Restitúyelo:  git checkout -- scripts/lib/fetch-doble.mjs'
  );
  process.exit(1);
}
...
casoRed('(aa) deriva de paginación: el mismo comentario en el corte de página ⇒ descartado por `id`, ANUNCIADO, y el sello sale SIN duplicados', {
  ...
  ledger: (t) => {
    ...
    // El conteo VIAJA en el sello, no solo en el `::warning` de la corrida que lo
    // produjo: es la única señal de que el corpus se MOVIÓ mientras se le miraba, y
    // por tanto el único proxy del salto por borrado (residual (a) de AP-076). Sin
    // esta línea, quitar el campo del sello no mata ningún caso.
    if (l.barrido.duplicados !== 1) return `el sello no registra la deriva: \`barrido.duplicados\` = ${JSON.stringify(l.barrido.duplicados)}, esperaba 1`;
...
// (dd) La deriva llega a la LÍNEA DEL VERDE y no muere en el `::warning` (🟡 3 de
// la review de AP-076). El duplicado va SIN marcador a propósito: lo que se mide
// aquí es el conteo del barrido, no la adjudicación — con `CORPUS_MARCADO` el caso
// sería rojo por (ii) y mediría otra cosa.
const RUIDO = com(9700, 1696, 'comentario sin marcador de capa ni de rol: ruido de barrido.');
casoRed('(dd) deriva SIN sellar ⇒ la línea del VERDE declara que el corpus se movió durante el barrido', {
  ledger: ledgerVigente([]),
  plan: [
    { relleno: 99, desde: 5000, items: [RUIDO] },
    { relleno: 2, desde: 4900, items: [RUIDO] },
  ],
}, { exit: 0, dice: ['verde', 'el corpus DERIVÓ durante el barrido', '1 comentario(s) repetido(s) en el corte de página'] });

// (ee) Y lo declara TAMBIÉN cuando el barrido de hoy no ha corrido, leyéndolo del
// sello vigente — que es el caso que importa dentro de tres meses: saber si el
// ledger que se está usando nació sobre un corpus quieto o sobre uno derivando.
// Misma mecánica que `truncado`, y por la misma razón.
caso('(ee) `duplicados` SELLADO ⇒ el verde lo declara aunque el barrido vivo esté OMITIDO', {
  ledger: ledgerVigente([], { duplicados: 3 }),
}, { exit: 0, dice: ['verde', 'barrido vivo OMITIDO', '3 comentario(s) repetido(s) en el corte de página'] });

// (ff) y (gg) — la mitad VERDE de las dos autodenuncias, que nadie mataba (pregunta
// abierta 2 de la review). (bb) y (k) fijan el rojo: un sello de banco leído en
// producción no adjudica nada. Pero endurecer cualquiera de los dos a INCONDICIONAL
// —quitando el `&& !DOBLE` o el `&& !FIXTURE`, que dejaría el modo banco inutilizable
// y todo este fichero muerto— no mataba ningún caso y el banco seguía verde. El
// hueco era simétrico y pre-existente para `fixture`; se cierra aquí para los dos.
casoRed('(ff) ledger `barrido.doble: true` leído CON el doble instalado ⇒ VERDE (el rojo de (bb) NO es incondicional)', {
  ledger: ledgerVigente([], { doble: true }), plan: [{ relleno: 2, desde: 5000 }],
}, { exit: 0, dice: ['verde'], calla: ['doble de banco', 'no adjudica nada'] });

caso('(gg) ledger `barrido.fixture: true` leído EN modo fixture ⇒ VERDE (el rojo de (k) NO es incondicional)', {
  ledger: ledgerVigente([], { fixture: true }), fixture: [com(9800, 1696, 'ruido de fixture')],
}, { exit: 0, dice: ['verde', 'corrida de FIXTURE'], calla: ['FIXTURE de banco', 'no adjudica nada'] });
...
console.log(
  ...
  'doble de `fetch` —paginación, `desc`, timeout, fail-open por HTTP y la deriva de paginación, que viaja al sello y a la ' +
  'línea del verde (residual (c) de AP-075, AP-076)—, más las DOS mitades (roja y verde) de cada autodenuncia de costura.'
);
